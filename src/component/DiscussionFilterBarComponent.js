...
import { LayoutAnimConf, Styling, t, useTheme, wait } from '../lib'

export const DiscussionFilterBarComponent = ({ discussionTitle, nyx, onFilter, onBack }) => {
  ...
  const searchUsername = async phrase => {
    ...
    setIsFetching(true)
    const res = await nyx.search({ phrase, isUsername: true })
        ...
        <View>
          <View style={Styling.groups.flexRowSpbCentered}>
            <IconButton
              ...
              rippleColor={colors.ripple}
            />
            <Text style={{ fontSize: fontSizes.p + 2, maxWidth: '70%' }} numberOfLines={1}>
              {discussionTitle}
            </Text>
            <Icon
              name={'search'}
